...
`src/data/scopeGroups.ts` (the PAT Link Builder's scope tree) isn't generated by
a script -- it imports `capabilities.json` directly and derives the grouping at
build time, so it can't go stale relative to it.

...
This section covers standards for handling book entries – from adding new books, setting titles, handling series info in titles, to dealing with special cases like multiple formats or partial works.

## Adding New Books 
If by searching Hardcover you cannot find a book that should exist, you can add it to the database. There are two methods for adding a book: via an identifier such as an ISBN or an external ID to a supported platform such as Goodreads, Open Library, or Google Books.

These functions are available to all users of Hardcover and the results are made immediately public to everyone.

If the book you're looking for exists, but a specific Edition does not, use [these instructions](../editionstandards/#) instead.


### Adding New Books via ISBN or External ID


### Adding new books Manually

...
- There is a known issue where an edition may appear to be assigned to a completely random book. We're working to pin down how this happens, but the fix for these is to edit the book and switch the cover edition to the correct one that's actually on the book.
...
- It is known that series merges do not remove and redirect the deduplicated series, causing it to be recreated on subsequent book imports.

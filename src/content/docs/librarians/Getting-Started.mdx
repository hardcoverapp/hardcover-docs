...
*This guide explains what the Librarian role is, who can apply, and how to get started as a contributor on Hardcover.*
...
As a prerequisite, you will need to join our <a href={URLS.DISCORD} target="_blank" rel="noreferrer noopener">Discord</a> server as a confirmed member (<a href={URLS.LINK_ROLES}target="_blank" rel="noreferrer noopener">instructions</a>).

Once roles have been linked, simply fill out the <a href={URLS.LIBRARIAN_APPLICATION} target="_blank" rel="noreferrer noopener">librarian application form</a> on our website. After the application has been submitted, the team will review it. 

If approved, you will receive an email with further instructions on how to get started as a librarian. You'll also be asked to <a href={URLS.LINK_ROLES} target="_blank" rel="noreferrer noopener">link</a> your new Librarian role to grant you access to our <a href={URLS.LIBRARIAN_DISCORD} target="_blank" rel="noreferrer noopener">#librarians</a> collaboration channel, where you can ask questions, get clarification on guidelines for handling metadata, and receive help from other librarians, senior librarians, and the Hardcover team.

:::note[Approval Timeline]
There's no set timeline for approval, but we do typically hold onto applications for a period of time if there is no history of reports to review. If you have been actively reporting issues around the site and have been waiting for longer than a month, feel free to ping `@team-librarians` on the server.
:::

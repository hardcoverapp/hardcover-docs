...
- `typos` - A comma separated list of numbers indicating the typo tolerance for each of the `fields` when calculating match (will show up in errors as `num_typos`)

<Aside type="note">
    `fields`, `weights` and `typos` are used together. If you pass in 2 fields to
    search (ex: `name,name_personal`), you'll also need to pass in 2 weights (`5,1`) and 2 typos (`2,1`).
...
<Aside type="note" title="Note #3">
    Modifier fields like `weights` and `typos` also accept one value,
    that value will then apply equally to all the `fields`.
</Aside>

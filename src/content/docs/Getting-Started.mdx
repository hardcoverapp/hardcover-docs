...
<Steps>
    ...
    4. Build something awesome and share it with us on [Discord](https://discord.gg/edGpYN8ym8)!
</Steps>

## Getting Help
If you have any questions or need help, please reach out to us on [Discord](https://discord.gg/edGpYN8ym8).
...
Later on, we hope to
allow developers to join a group that allowlists specific sites, but that's a ways down the line.

## Limitations
- API tokens automatically expire after 1 year, and reset on January 1st.
- <Badge text="Dec 2024" variant="success" /> API is rate-limited to 60 requests per minute.
- <Badge text="Dec 2024" variant="success" /> The following queries are disabled:
    - `_like`
    - `_nlike`
    - `_ilike`
    - `_niregex`
    ...
    - `_nsimilar`
    - `_similar`
- <Badge text="Dec 2024" variant="success" /> Queries have a max timeout of 30 seconds.
- <Badge text="Dec 2024" variant="success" /> Queries are not allowed to run in the browser, they must be run in an environment where the token can be kept secure.
- <Badge text="Feb 2025" variant="caution" /> Queries have a maximum depth of 3.
- <Badge text="Feb 2025" variant="caution" /> Queries are limited to your own user data, public data, and user data of users you follow.
- <Badge text="Early 2025" variant="note" /> OAuth support will be added for external applications.

## Want to Contribute?
We are actively looking for contributors to help us improve the API documentation.

For more information about how to contribute to the API Documentation, please see the [Contributing Guide](/contributing).

#!/usr/bin/env bash
# Needs curl and openssl. Paste this in block by block, or run the whole
# thing with `bash oauth.sh`.
...
# nothing to do -- state and verifier are already sitting in $STATE and $VERIFIER
...
echo "open that URL, approve, then copy code/state/iss out of the address bar"
...
TOKEN_RESPONSE=$(curl -s -u "${CLIENT_ID}:${CLIENT_SECRET}" "$TOKEN_ENDPOINT" \
...
curl -s -u "${CLIENT_ID}:${CLIENT_SECRET}" "$REVOKE_ENDPOINT" \
